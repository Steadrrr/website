<?php
/**
 * 탭 화면: 상단 메뉴 + 페이지들을 탭(최대 5개)으로 띄우는 틀.
...
<script>window.FORESTLOG_TABS = { max: 5, home: <?= json_encode(url('index.php')) ?>, shell: <?= json_encode(url('shell.php')) ?>, site: <?= json_encode(' · ' . config('site_name', '휴양림 업무일지')) ?> };</script>
