<?php
/** 근무 설정 (관리원): 입사일·계약종료일·근무시간·휴무 요일 — 최고관리자·팀장 */
...
if (is_post()) {
    ...
    if ($end !== '' && (!valid_date($end) || $hire === '' || $end < $hire)) $errors[] = '계약 종료일은 입사일 이후 날짜로 입력하세요.';
...
<form method="post" class="card narrow">
  ...
  <div class="row">
    <label>입사일<input type="date" name="hire_date" value="<?= e($u['hire_date']) ?>"></label>
    <label>계약 종료일 <small class="muted">(비우면 입사일 + 1년)</small><input type="date" name="contract_end" value="<?= e($u['contract_end']) ?>"></label>
  </div>
  <div class="row">
  ...
  <?php endif ?>
  <p class="muted small">연차는 입사일부터 1개월 만근(결근 없음)마다 1일 발생하며, 최대 발생 연차(계약 개월 수, 최대 11일)까지 당겨 쓸 수 있습니다.
